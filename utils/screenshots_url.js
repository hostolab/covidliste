...
const work = async(worker) => {
  ...
  try {
    browser = await puppeteer.launch({
      ...
      args: ['--no-sandbox', '--disable-setuid-sandbox', '--disable-gpu', '--window-size=850,2160','--window-position=0,0'],
    ...
    await page.goto(worker["url"], { timeout: 15000, waitUntil: 'networkidle2' });
    const frameElement = await page.waitForSelector('iframe[name="messageBody"]')
    ...
    var bannerHeight = await page.evaluate(() => {
      ...
      var domHeaderItems = [...document.querySelectorAll('body > header dt')].filter(e => (e.textContent.includes('SMTP-To') || e.textContent.includes('Date') || e.textContent.includes('Format') || e.textContent.includes('Locale') || e.textContent.includes('To')))
      var domHeader = document.querySelector('body > header');
      if (domBullets.length > 0) {
        Array.prototype.forEach.call(domBullets, function(node) { node.parentNode.removeChild(node); });      
      }
      ...
      return Math.max(domHeader.offsetHeight, domHeader.clientHeight, domHeader.scrollHeight);
    });
    var iframeHeight = await iframe.evaluate(() => {
      ...
      return Math.max(domContent.offsetHeight, domContent.clientHeight, domContent.scrollHeight);
    });
    var documentHeight = iframeHeight + bannerHeight;
    // console.log("iframeHeight: " + iframeHeight);
    // console.log("bannerHeight: " + bannerHeight);
    // console.log("documentHeight: " + documentHeight);
    await page.setViewport({ width: 850, height: documentHeight, deviceScaleFactor: 2 })
    ...
    doc.fontSize(18).fillColor("#d9215f").text(`[${worker["locale"]}] ${worker["name"]}`, 20, 20)
    doc.image(img, 0, 40);
