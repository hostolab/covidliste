namespace :batches do
  ...
  task :send, [:batch_id] => [:environment] do |t, args|
    ...
    batch.matches.each do |match|
      next if (match.sms_sent_at.present? or match.mail_sent_at.present?) 
      ...
      begin
        response = client.messages.create({
          ...
          body: "Bonne nouvelle ! Un vaccin #{batch.campaign.vaccine_type} est disponible. Réservez-le avant #{match.expires_at.strftime("%Hh%M")} en cliquant ici : https://covidliste.com/matches/#{match.match_confirmation_token}"
        })
        puts response.body
