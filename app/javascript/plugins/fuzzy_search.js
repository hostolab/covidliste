...
if (engine) {
  ...
  let domInput = document.querySelector(engine.getAttribute("data-fuzzy-search-input"));
  let domNoResults = document.querySelector(engine.getAttribute("data-fuzzy-search-none"));
  let domItems = document.querySelectorAll(engine.getAttribute("data-fuzzy-search-item"));
  let domItemTitleSelector = engine.getAttribute("data-fuzzy-search-item-title");
  let domItemCategorySelector = engine.getAttribute("data-fuzzy-search-item-category");
      ...
      .then((dataset) => {
        ...
        domInput.addEventListener("keyup", function(e) {
          ...
          if (searchTerm.length < 3) {
            ...
            for (var i = 0; i < domItemsCount; i++) domItems[i].classList.remove("d-none");
          } else {
            engine.classList.add("search-done")
            const matches = fuse.search(searchTerm);
            console.clear();
            console.table(matches);
            const matchesFiltered = matches.filter(match => match.score <= trustScoreFactor);
            const matchesTitles = matchesFiltered.map((r) => r.item["title"]);
            if (matchesTitles.length > 0) {
              domNoResults.classList.add("d-none");
              for (var i = 0; i < domItemsCount; i++) {
                const domItemTitle = domItems[i].querySelector(domItemTitleSelector).textContent.replace(/^\s+|\s+$/g,'');
                if (searchTerm.length == 0 || matchesTitles.indexOf(domItemTitle) >= 0) {
                  domItems[i].classList.remove("d-none");
                  domItems[i].style.order = matchesTitles.indexOf(domItemTitle);
                } else {
